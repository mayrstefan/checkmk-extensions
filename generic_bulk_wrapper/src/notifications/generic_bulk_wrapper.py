...
def call_script(script_env: dict[str, str]) -> NoReturn:
    ...
    if "NOTIFY_PARAMETER_1" not in script_env:
        sys.stderr.write(
            "First parameter must be the notification script to call: "
            "parameter is empty"
        )
        sys.exit(1)
    ...
    script_env["NOTIFY_PARAMETERS"] = script_env["NOTIFY_PARAMETERS"][
        len(script_name) :
    ].lstrip()
...
if __name__ == "__main__":
    ...
    if not BULK_MODE:
        ...
        for context in contexts:
            env_context = {
                f"NOTIFY_{ k }": limit_str(v)
                for k, v in context.items()
                if not k.startswith("OMD_")
            }
